...
internal sealed class TwitchAuthStatusProbe(HttpClient client)
{
    public async Task<TwitchAuthStatusProbeResult> ProbeAsync(CancellationToken cancellationToken = default)
    {
        ...
        return status is null
            ...
            : TwitchAuthStatusProbeResult.Success(status.ClientIdConfigured, status.HasRefreshToken);
    }
...
internal sealed record TwitchAuthStatusProbeResult(
    ...
    string? ErrorCode)
{
    public static TwitchAuthStatusProbeResult Success(bool clientIdConfigured, bool hasRefreshToken)
    {
        return new TwitchAuthStatusProbeResult(true, clientIdConfigured, hasRefreshToken, null);
    }

    public static TwitchAuthStatusProbeResult Failure(string errorCode)
    {
        return new TwitchAuthStatusProbeResult(false, false, false, errorCode);
    }
}
