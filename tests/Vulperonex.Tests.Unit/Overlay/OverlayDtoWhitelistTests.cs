...
public sealed class OverlayDtoWhitelistTests
{
    ...
    {
        var payload = new OverlayMemberPayload(1, "evt-1", DateTimeOffset.UnixEpoch, "Alice", "avatar", 3, 1, 3);

        SerializeKeys(payload).Should().BeEquivalentTo("schemaVersion", "eventId", "timestamp", "displayName", "avatarUrl", "checkInCount", "roundIndex", "stampSlotInRound");
    }
