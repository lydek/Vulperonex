...
Phase 5.5 的目標：**讓 Phase 6 開始前，CLI 與既有 overlay 已能驅動單條完整的 chat→workflow→overlay 鏈，無外部工具相依**。
...
- **`member-card.html` 移植**：相依尚未定義的 `/overlay/member` 完整 DTO 與 SystemEvent；屬 Phase 6。
...
- `rule update` 對於 PUT 失敗（驗證錯誤）時要不要回應 `meta`：Phase 5 既有 `WriteResponseAsync` 只印 `error` code；若 cookbook 顯示「為何更新失敗」是必要，需擴充 stderr 格式，列入待議，**不**在 5.5 實作。
