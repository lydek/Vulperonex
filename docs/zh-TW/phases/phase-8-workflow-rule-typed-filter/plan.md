...
>
> **產出範圍**：Spec 第 6 節 Phase A → Phase E 之全部實作切片、相依、驗證、Checkpoint。
...
| AD-2 | `ExpressionContext` 加 `RuleId/RuleName` 屬性（非改 `IExpressionEvaluator` 簽名） | 介面穩定，相容既有測試 |
...
- [x] 編輯器只渲染 metadata 定義的 typed 欄位，不相依 `migrationWarnings` 或任何 legacy filter 清理流程
...
| ❌ | Phase B → Phase C（C 相依 metadata 合法清單） |
