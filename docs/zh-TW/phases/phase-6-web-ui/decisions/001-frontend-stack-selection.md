...
Phase 6 將已驗證的本機 Web API、SignalR Overlay、CLI 手動測試流程以及 Twitch OAuth 狀態移入本機 Web UI，隨後將該 UI 封裝在 Photino 桌面外殼中。第一個畫面必須是操作主控台，而不是登入頁面。

該技術棧必須支援：

- 使用 TypeScript 的 Vue 單一檔案元件 (SFC)。
- 用於頻繁實況主操作的緊湊本機管理 UI。
- 與管理狀態隔離的 Overlay 路由。
...
- **PrimeVue 4 Unstyled**：用於具備無障礙支援的元件基礎元件，而不強制使用視覺主題。
...
- **Vitest + Vue Test Utils**：用於元件和 Composable 測試。
