...
> **⚠ OAuth Gate 注意**：「真實瀏覽器授權」需包含完整 code exchange + refresh_token 加密儲存，不僅是 `auth start` 開啟瀏覽器。若 Phase 5 manual-verification.md 僅記錄開啟授權 URL，須在有效 `Twitch:ClientId` 環境補充完整 OAuth round-trip 驗收後方可通過此 Gate。
...
- **Phase 6 實作時程**：**7 天** (II18)
  ...
  - Task 20 五大管理面板 (含 1MB JSON 貼上防護、LWW 重複抑制、指數退避)：**3 天**
  - Task 18 Serilog 三 Sink 與 AppLogs 清理機制：**1 天**
  - Task 22 Overlay 事件歷史持久化 + 清理 surface (重用 SystemSettings + Omni-Commander pattern)：**0.5 天**
...
Phase 6 把 Phase 5 已由 CLI 驗證過的 loopback Web API 能力搬到 Vue Web UI，形成可長時間操作的本機主控台。第一個畫面必須是可用的操作介面，而不是 landing page：使用者可以確認 API/Twitch 狀態、模擬事件、檢視會員、管理 WorkflowRule、監看 SignalR 事件，最後由 Photino Desktop Shell 包裝成桌面入口。
...
- [ ] retention cleanup 與 size cleanup 可單次透過呼叫 `AppLogsCleanupWorker.ExecuteOnce()` 觸發測試，不相依 background timing。
...
- 全部 stack 套件（Vue 3.5、Vite 7.3、PrimeVue 4 Unstyled、UnoCSS Preset Wind 4、Pinia、vue-i18n、oxlint、vue-tsc 以及所有傳遞相依）首次安裝前須取得使用者同意（ask-first 協定）。
...
- [ ] **useEventStore Setup Store 設計 (II7, II10)**：
  ...
  - **命名與單向資料流規範 (II10)**：Pinia store 統一採用 Setup Store 語法，命名規則必須為 `use[Feature]Store` (例如 `useAuthStore`、`useWorkflowStore`、`useMonitorStore`)。狀態必須以唯讀屬性 (`readonly(state)`) 露出，且僅能透過 Store Actions 進行修改，維持 unidirectional data flow (單向資料流)。
  - **資料重複抑制與一致性 (Last-write-wins) (II7)**：Store 內以 `eventId` 為 key 進行儲存與維護。由 `useEventStore` 作為 **Last-write-wins reducer** 的實作位置。當 SignalR 與 HTTP Polling fallback 同時傳來相同 `eventId` 的事件時，必須比對 envelope 的 `occurredAt`，採用 `last-write-wins` (LWW) 覆蓋策略，將最新 occurredAt 的內容寫入 store。
  - **Overlay 獨立與解耦設計**：Overlay（`/overlay/chat`, `/overlay/alerts`）不共享 `useEventStore` 的狀態。它們應直接並單獨呼叫各自的 `useOverlayHub(hubName)` composable 連線對應 of `/hubs/overlay/chat` 與 `/hubs/overlay/alerts` 獨立 Hub。這能確保 Overlay 的安全性邊界（僅消費白名單欄位，不將敏感的管理主控台資料帶入 Overlay 頁面，防範 DTO 溢位）。
...
**描述：** 實作瀏覽器內可操作的核心工作流。此任務之所有前端主控台 View 與 Component 的路徑均扁平化為 `src/frontend/src/views/admin/` 與 `src/frontend/src/components/admin/`，取代多層 nested 目錄（BB3, CC1, CC2, CC3）。本任務讓使用者無需 CLI 即可完成 Phase 5 manual verification 的主要步驟：模擬事件、確認 member side effect、建立/啟用/停用/刪除 rule、重設/啟動 Twitch OAuth，並實作完整的健全性防護機制。
...
- [ ] **會員唯讀負向測試斷言 (Z10)**：所有會員欄位（如姓名、平台識別碼等）在 Web UI 內均為唯讀，不允許在前端直接編輯，且不可出現 seed/delete 操作入口。
...
- [ ] **JSON Textarea 1MB limit 三重 check (II15)**：
  ...
  - 長度在貼上（paste）時進行 `300ms` 防抖動（debounce）檢查，當資料大小超過 1MB 時，拒絕解析並顯示 toast 警告。
  - 將貼上的原始大文字存入非響應式變數（如 plain object 或自訂普通 ref/變數），而非直接指派給 Vue 的響應式 ref，防範 Vue 反覆偵測屬性變化而造成的主執行緒卡死與 OOM 崩潰。
...
- [ ] Twitch panel 支援 auth start/reset。**Twitch OAuth 302 重導向回根路徑 (II4, II29)**：Twitch OAuth 授權成功或失敗後，OAuth callback endpoint 必須先在後端消費 `code`、完成 token exchange 與 refresh token 加密儲存，再以 `302` 重導向回本機 Web UI 根路徑（`/`）。Web UI 不接收 OAuth `code` 或 raw error；授權結果由 `platform.connection_changed`、`GET /api/twitch/status` 與 toast/status card 呈現。
- [ ] **Twitch Reset 與 emit 變更 (II25)**：執行 Twitch Reset 時，除了清除後端 refresh token，還必須主動斷開與 Twitch 的連線，並向所有訂閱的 Overlay Hubs/Web Clients 傳送狀態變更 event 以重設狀態。
...
- [ ] Vitest：member list empty/success/error states，並實作負向測試斷言，確保 Web UI 無 seed/delete 入口且所有會員欄位唯讀 (Z10)。
- [ ] Vitest：rule enable/disable/delete 成功後更新 local state；測試 409 樂觀鎖衝突的捕捉與 UI 提示。
- [ ] Vitest：測試 JSON textarea 1MB 貼上防抖動防崩潰機制，確保大於 1MB 的資料會被拒絕且不寫入響應式狀態。
...
- [ ] Browser manual：缺 Twitch ClientId 時 UI 顯示 no-Twitch mode；有 ClientId 時 auth start 開啟 Twitch URL，授權完成後後端完成 code exchange、refresh token 加密儲存，並 302 重導向回 `/`，Web UI 透過 status/event 顯示成功狀態（依 `manual-verification.md` § Task 20k）。

**相依：** Task 19, Task 14a, Task 14b, Phase 5 Task 16f/16g manual gates
...
**描述：** Overlay (chat / alerts / member) 事件目前完全 in-memory，SPA 換 route、F5、應用重開、OBS browser source 重新載入皆會清空。Task 22 引入跨重新啟動可 rehydrate 的小型 ring buffer，並提供管理端與 overlay 端的清理入口，讓「測試驗證歷史」與「OBS 重連補畫面」可用。設計直接重用 Omni-Commander `ChatHistoryService` pattern (`ref/Omni-Commander/OmniCommander.Infrastructure/Overlay/ChatHistoryService.cs`)：以 `SystemSettings` 表單行 JSON 全量寫入，無需新表/migration/index。

**驗收標準：**
- [ ] **儲存載體**：重用既有 `SystemSettings` 表，三個 key：`Overlay.History.Chat` / `Overlay.History.Alerts` / `Overlay.History.Member`。`ValueJson` 為對應 payload list 全量序列化（cap 內，序列化大小應 < 10KB）。**不新增 EF entity / migration**。
...
**相依：** Task 19 (frontend skeleton)、Task 20a (simulate 已存在便於驗證)。建議在 Task 20b (event monitor) 之前完成，因 history endpoint 與 dedupe 思路 monitor 可沿用。
...
**規模：** M（重用既有 SystemSettings + Omni-Commander pattern，無 migration）
...
| Twitch OAuth 從 Web UI 啟動與 CLI loopback callback 語意混淆 | 中 | Web UI 只透過 Web API Twitch auth endpoints，不直接儲存 token；缺 ClientId 時 fail closed |
...
| 目前 Phase 5.5 CLI id resolution worktree 尚未收斂 | 低 | Phase 6 docs 不相依該 dirty diff；實作 Task 20 前再確認 API/CLI 最終命令語意 |
