...
- 主控台（Dashboard）：API 狀態卡片顯示為綠色；Twitch 狀態卡片正確顯示 `clientIdConfigured` / `hasRefreshToken` / `connected` 狀態。
...
- Twitch 面板（無 ClientId）：顯示 no-Twitch mode，且 auth start 按鈕停用。
- Twitch 面板（有 ClientId）：在 Photino 桌面環境中 click auth start 會以系統預設瀏覽器開啟 Twitch 授權 URL；在 Web UI 瀏覽器環境中則在同一個分頁中重新導向。這兩種行為都必須驗證。
...
2. 在瀏覽器中授權 → Twitch 重新導向至 `http://localhost:7979/auth/callback`。
...
5. 驗證 Web UI Twitch 面板透過 `platform.connection_changed` SignalR 事件自動重新整理。
...
- 佔用全部 5 組連接埠對（5000-5009） → Photino 對話方塊顯示無可用連接埠錯誤。
- 無 WebView2 → 顯示包含下載連結的對話方塊。
- EF Core 遷移失敗 → 顯示包含 [開啟日誌資料夾] 與 [結束] 的對話方塊。
- Web 主機崩潰 → 顯示內嵌 fallback HTML 與重新啟動按鈕；自動重新啟動最多嘗試 3 次。
- 重新啟動上限：強制 Web 主機連續崩潰 4 次 → 驗證在第 4 次崩潰時重新啟動按鈕停用，並顯示清晰的提示「請手動重新啟動應用程式」。
...
- 驗證者：lydek
- 環境：Windows 11, .NET 10.0.203, 透過 corepack 使用 pnpm@9.15.4, Chrome
- 命令 / 步驟：執行 `## Task 22 Overlay History Persistence Checklist` 下的八項檢查（跨路由持久化、跨重新啟動重新載入、提醒重播不播動畫、上限限制、標頭按鈕清空、狀態頁按鈕清空、確認彈出視窗取消路徑、會員 hub 清空）。
- 預期結果：八項檢查均符合預期；重新啟動 Web 主機後，SystemSettings 中的 `overlay.history.{chat,alerts,member}` 列依然留存；提醒重播時標記為 `data-replayed="true"` 並略過橫幅動畫。
...
- 實際結果：所有檢查皆通過。模擬確認（ack）包含可追蹤的 `eventId`；overlay 推送確認；會員列表無修改控制項；規則生命週期切換正常，含二次確認對話方塊；事件監看器即時更新。
- 結果：PASS
- 憑證 / commit：5846895
...
- 命令 / 步驟：
  ...
  3. 驗證程式碼細節與 UX：
     ...
     - IPC DTO 契約透過單元測試驗證，資料結構為 `{ type, payload }`。
