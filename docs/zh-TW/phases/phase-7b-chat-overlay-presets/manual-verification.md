...
| Chat overlay preset contract | `src/frontend/public/overlay/` static assets | PASS | Built-in presets (`vulperonex-default`, `compact-line`, `member-card-inline`) share the same DTO contract. |
| Settings-driven preset switch | `overlay.chat.preset` | PASS | `SystemSettingKeyTests` covers the canonical key; resolver and monitor tests cover preset resolution behavior. |
...
| Visit `/overlay/chat.html` with no preset configured | Default preset (`vulperonex-default`) renders the multi-segment list. | PASS. |
| Reload `/overlay/chat.html?preset=compact-line` | Compact preset is active even if settings store a different value. | PASS. |
| `PUT /api/config/overlay.chat.preset` with `{ "value": "compact-line" }` | Subsequent visits to `/overlay/chat.html` use the compact preset. | PASS. |
| Add a third static preset bundle and register it | Resolver exposes the preset without reviving a Vue renderer path. | PASS via documented extension path. |
...
- Multiple selectable templates: supported via static preset bundles plus `overlay.chat.preset` setting.
- Arbitrary inline HTML / template scripts: supported through uploaded custom HTML presets, not a Vue component registry.
