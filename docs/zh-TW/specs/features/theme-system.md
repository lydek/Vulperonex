...
**目標：** Vulperonex 管理 UI 必須支援全域主題模型，而非依賴散落且硬編碼的亮色。主題系統涵蓋 Vue 管理外殼、共享管理元件、監控器框架 (monitor chrome)、設定 UI 以及嵌入式預覽框架。OBS 疊層預設集 (OBS overlay presets) 除非渲染為管理框架，否則保持預設集範圍的獨立樣式。
