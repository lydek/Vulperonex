# Vulperonex 快速測試：聊天回顯規則調試指南 (Cookbook)

本指南引導開發者與測試代理進行 E2E 完整聊天回顯鏈的調試，從啟動服務器、建立規則、訂閱 SignalR Overlay 直到發送模擬訊息並觀察回顯結果。
...
啟動本機 API 與雙埠宿主。服務器將自動套用 SQLite 資料庫移轉，無需手動執行移轉命令。
* **命令**：
  ```powershell
  # 切換至專案根目錄
...
* **觀察點與通過條件**：
  | 觀察對象 | 通過條件 / 預期結果 |
  | :--- | :--- |
  | 控制台日誌 | 顯示 Kestrel 成功綁定至 Loopback 雙端點（例如 `http://127.0.0.1:5000` 與 `http://[::1]:5000`） |
  | 資料庫狀態 | 若是新環境，自動在資料庫路徑建立 SQLite `Vulperonex.db` 並完成 table 初始化 |

---

### 步驟 2：使用 CLI 建立聊天回顯規則 (Create Rule)
使用 CLI 工具載入預設的最小回顯規則檔案 `rule-chat-echo.json`。
* **命令**：
  ```powershell
...
* **測試方法**：
  執行 `tests/Vulperonex.Tests.Integration/RapidTest/ChatReplyChainTests.cs`。該測試自動化複製了上述步驟 1 至 5 的全套流程，並對 SignalR 即時回顯負載、ULID 與 outbox side-effects 進行強斷言。
* **驗證命令**：
  ```powershell
  rtk dotnet test --filter "FullyQualifiedName=Vulperonex.Tests.Integration.RapidTest.ChatReplyChainTests"
  ```
* **預期結果**：
  SignalR Client 在 5 秒內精確捕獲 `"Echo: hello"` 訊息，且 outbox 保存此回顯。
* **實際結果**：
...
* **測試方法**：
  人肉啟動 Web API，透過 PowerShell 連接 REPL 互動終端，輸入 `rule create`、`simulate chat`，並在瀏覽器中人眼觀察 Overlay 渲染；同時成功完成 Twitch PKCE OAuth 授權與 SQLite 加密儲存。
