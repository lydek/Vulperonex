...
- [ ] Task 21e：Web host crash 偵測與內嵌 fallback HTML 呈現。**重新啟動次數上限與 Vitest 斷言 (II13)**：模擬 Web host crash 的重新啟動行為，前 3 次 crash 會自動 retry 重新啟動，到第 4 次 crash 時，停止 retry，並在 UI fallback 畫面提示「多次重新啟動失敗，請手動重新啟動 Vulperonex 服務」。補齊 mock Web host 單元測試，在單元測試中斷言前 3 次自動重新啟動、第 4 次停止並提示手動重新啟動的行為符合預期。
...
- [ ] Task 21g：.NET 10.0 + Photino 3.x 相容性預驗 (II30)：執行 compatibility 預驗，若發生 native runtime 崩潰時提供「切回 WebView2 fallback 或改以獨立 Kestrel 服務執行」之緩解手段。
