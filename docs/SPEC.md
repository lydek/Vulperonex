...
| `UserDonatedEvent` | `TotalBitsGiven = event.TotalBitsGiven`（**絕對值替換**，非 `+= amount`；event payload 攜帶 cumulative total，TDQ 重播不重複累加）|
...
| 鍵 | 類型 | 範例值 |
|---|---|---|
| `display.color` | 十六進位字串，格式 `^#[0-9A-Fa-f]{6}$`（6 位 RGB，不接受 CSS function、named color、8 位 alpha 或空字串）| `#FF4A4A` |
| `display.segments` | JSON 片段陣列 | 見下方 |
| `user.avatar` | URL 字串 | `https://cdn.twitch.tv/...` |
| `user.badges` | 逗號分隔的徽章 ID；每個 badge ID 字元集 `[A-Za-z0-9_/\-]`；最多 20 個；重複項去重（保留首次出現順序）| `subscriber/2000,vip` |
...
OAuth PKCE 流程：
  ...
  4. 應用程式驗證 state 與步驟 1 相同（CSRF 防護）；不符 → 拒絕，記 warning，不 exchange code
  ...
  - access_token 不寫 log；refresh_token 以明文傳入 IOAuthTokenStore，由 Task 8 加密
...
| 疊層 | 允許的欄位 | 禁止的欄位 |
|---|---|---|
| `/overlay/chat` | DisplayName, ColorHex, Segments, Badges | MemberId, UserId, TotalBitsGiven |
| `/overlay/alerts` | DisplayName, EventType, Tier | MemberId, PlatformUserId |
| `/overlay/member` | DisplayName, AvatarUrl, CheckInCount (僅限當前會話) | MemberId, TotalLoyalty, LinkedPlatforms |

`OverlayModule` 在 SignalR 推送前將領域事件轉換為這些受限的 DTO。允許列表在 DTO 類型級別強制執行 — 無動態映射。
